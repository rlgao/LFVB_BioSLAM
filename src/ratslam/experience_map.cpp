...
ExperienceMap::ExperienceMap(ptree settings)
{
  ...
  get_setting_from_ptree(EXP_LOOPS, settings, "exp_loops", 10);  // 20
  get_setting_from_ptree(EXP_INITIAL_EM_DEG, settings, "exp_initial_em_deg", 0.0);  // 90 140
  ...
  accum_delta_facing = EXP_INITIAL_EM_DEG * M_PI / 180.0;
  accum_delta_x = 0;
  accum_delta_y = 0;
...
// create a new experience(node) for a given position 
int ExperienceMap::on_create_experience(unsigned int exp_id)  // action->dest_id
{
