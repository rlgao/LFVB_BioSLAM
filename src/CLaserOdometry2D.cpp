...
void CLaserOdometry2D::setLaserPose(const Pose3d& laser_pose)
{
  ...
  // std::cout << std::endl;
  // std::cout << laser_pose_on_robot_.matrix() << std::endl << std::endl;
}
                            ...
                            const geometry_msgs::Pose & initial_robot_pose)
{
  ...
  // ROS_INFO_STREAM("scan.angle_max: " << scan.angle_max);
  // ROS_INFO_STREAM("scan.angle_min: " << scan.angle_min);
  // ROS_INFO_STREAM("fovh: " << fovh);
// [ INFO] [1679283480.996370419]: scan.angle_max: 3.14159
// [ INFO] [1679283480.996379692]: scan.angle_min: -3.12414
// [ INFO] [1679283480.996385809]: fovh: 6.26573


  ctf_levels = 5;   // Coarse-to-Fine levels
  iter_irls  = 5;   // Num iterations to solve 'iterative reweighted least squares' (IRLS)
  ...
  // ROS_INFO_STREAM("pyr_levels: " << pyr_levels);
  ...
  for (unsigned int i = 0; i < pyr_levels; i++)
  {
    ...
    range[i]     = Eigen::MatrixXf::Constant(1, cols_i, 0.f);  // row vector
...
bool CLaserOdometry2D::odometryCalculation(const sensor_msgs::LaserScan& scan)
{
  ...
  range_wf = Eigen::Map<const Eigen::MatrixXf>(scan.ranges.data(), width, 1);  // column vector
...
void CLaserOdometry2D::calculaterangeDerivativesSurface()
{
  ...
  for (unsigned int u = 1; u < cols_i-1; u++)
    dtita(u) = (rtita(u-1)*(range_inter[image_level](u+1) - range_inter[image_level](u)) 
              + rtita(u)*(range_inter[image_level](u) - range_inter[image_level](u-1)))
              /(rtita(u)+rtita(u-1));
...
bool CLaserOdometry2D::filterLevelSolution()
{
  ...
  if (eigensolver.info() != Eigen::Success)
  {
    ...
    ROS_ERROR_COND(verbose, "[rf2o] ERROR: Eigensolver couldn't find a solution. Pose is not updated");
    return false;
  }
...
void CLaserOdometry2D::PoseUpdate()
{
  ...
  ROS_INFO_COND(verbose, "[rf2o] LASERodom = [%f %f %f]",  // x, y, theta
  ...
  ROS_INFO_COND(verbose, "[rf2o] BASEodom = [%f %f %f]",  // x, y, theta
