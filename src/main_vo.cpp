...
void image_callback(sensor_msgs::ImageConstPtr image)
{
  ...
  // ROS_INFO_STREAM("greyscale: " << greyscale);

  // vo->on_image(&image->data[0], (image->encoding == "bgr8" ? false : true), // bool greyscale
  //             image->width, image->height, 
  //             &odom_output.twist.twist.linear.x, &odom_output.twist.twist.angular.z);
  vo->on_image(&image->data[0], greyscale,
              image->width, image->height, 
              &odom_output.twist.twist.linear.x, &odom_output.twist.twist.angular.z);
...
int main(int argc, char * argv[])
{
  ...
  std::string topic_root = "";  // "stlucia", "shihushan"
  ...
  {
    sub = it.subscribe(topic_root + "/rotated_image", 1, image_callback);
  }
  else
  {
    sub = it.subscribe(topic_root + "/camera/image", 1, image_callback);
  }
  /********************************************************/
