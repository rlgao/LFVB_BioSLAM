...
ros::Publisher pub_em;    // complete representation of the experience map as a topological map
ros::Publisher pub_pose;  // pose of the robot within the experience map
ros::Publisher pub_em_markers;  // rendering the map within rviz
...
{
  curr_odom_time = odo->header.stamp;
...
void action_callback(ratslam_ros::TopologicalActionConstPtr action, ratslam::ExperienceMap *em)
{
  ...
  // static int times;
  // ROS_INFO_STREAM("action_callback: " << times);
  // times++;
  ...
  em->iterate();  // graph relaxing


  // ----------------------------------------------------------
  // publish [pose]
  // pose_output.header.stamp = ros::Time::now();
  pose_output.header.stamp = curr_odom_time;
  pose_output.header.seq++;
  pose_output.header.frame_id = "odom";  //"map";
  ...
  pose_output.pose.orientation.z = sin(em->get_experience(em->get_current_id())->th_rad / 2.0);  // sin(yaw / 2)
  pose_output.pose.orientation.w = cos(em->get_experience(em->get_current_id())->th_rad / 2.0);  // cos(yaw / 2)
  pub_pose.publish(pose_output);


  // std::cout << "[RatSLAM] [x y yaw]: " << em->get_experience(em->get_current_id())->x_m
  //                               << " " << em->get_experience(em->get_current_id())->y_m
  //                               << " " << em->get_experience(em->get_current_id())->th_rad
  //                               << std::endl;

  ROS_INFO_STREAM(
    "[RatSLAM] [x y yaw]: " 
    << em->get_experience(em->get_current_id())->x_m
    << " " << em->get_experience(em->get_current_id())->y_m
    << " " << em->get_experience(em->get_current_id())->th_rad
  );
  ...
  // ----------------------------------------------------------
  // publish [experience map marker]
  em_marker.header.stamp = curr_odom_time;
  em_marker.header.seq++;
  em_marker.header.frame_id = "odom";  //"map";

  em_marker.type = visualization_msgs::Marker::LINE_LIST;
  em_marker.action = visualization_msgs::Marker::ADD;
  em_marker.scale.x = 0.1;  // no y, z

  em_marker.color.a = 1.0;
  em_marker.color.r = 1.0;
  em_marker.color.g = 0.0;
  em_marker.color.b = 0.0;
...
int main(int argc, char * argv[])
{
  ...
  std::string odom_topic = "/rf2o/odom";
  ros::Subscriber sub_odometry = node.subscribe<nav_msgs::Odometry>(odom_topic /*topic_root + "/odom"*/, 0, 
