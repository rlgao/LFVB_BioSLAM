...
void odo_callback(nav_msgs::OdometryConstPtr odo, ratslam::PosecellNetwork *pc, ros::Publisher * pub_pc)
{
  ...
  if (prev_time.toSec() > 0)
  {
    ...
    pc_output.src_id = pc->get_current_exp_id();  // current_exp
    ...
    if (pc_output.action != ratslam::PosecellNetwork::NO_ACTION)
    {
      ...
      ROS_DEBUG_STREAM("PC:action_publish{odo}{" << ros::Time::now() 
                        << "} action{" << pc_output.header.seq << "}=" <<  pc_output.action 
                        << " src=" << pc_output.src_id << " dest=" << pc_output.dest_id);
    }
...
int main(int argc, char * argv[])
{
  ...
  ros::Subscriber sub_odometry = node.subscribe<nav_msgs::Odometry>(
                                odom_topic /*topic_root + "/odom"*/, 0, boost::bind(odo_callback, _1, pc, &pub_pc),
