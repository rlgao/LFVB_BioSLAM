...
LocalViewMatch::LocalViewMatch(ptree settings)
{
  ...
  get_setting_from_ptree(VT_SHIFT_MATCH, settings, "vt_shift_match", 25);  // 4
  get_setting_from_ptree(VT_STEP_MATCH, settings, "vt_step_match", 5);     // 1
  get_setting_from_ptree(VT_PANORAMIC, settings, "vt_panoramic", 0);

  get_setting_from_ptree(VT_MATCH_THRESHOLD, settings, "vt_match_threshold", 0.03);  // 0.03
  get_setting_from_ptree(TEMPLATE_X_SIZE, settings, "template_x_size", 1);         // (num) 60
  get_setting_from_ptree(TEMPLATE_Y_SIZE, settings, "template_y_size", 1);         // (num) 20
  ...
  get_setting_from_ptree(IMAGE_VT_Y_RANGE_MAX, settings, "image_crop_y_max", -1);  // 150
                              ...
                              unsigned int image_width, unsigned int image_height)
{
  ...
  convert_view_to_view_template(greyscale);  // compute current_view and current_mean
  prev_vt = get_current_vt();  // return current_vt;

  unsigned int vt_match_id;
  compare(vt_error, vt_match_id);

  if (vt_error <= VT_MATCH_THRESHOLD)  // previously seen template (match)
  {
...
// store view template in [current_view]
void LocalViewMatch::convert_view_to_view_template(bool greyscale)
{
  int data_next = 0;
  int sub_range_x = IMAGE_VT_X_RANGE_MAX - IMAGE_VT_X_RANGE_MIN;  // 416
  int sub_range_y = IMAGE_VT_Y_RANGE_MAX - IMAGE_VT_Y_RANGE_MIN;  // 150
  int x_block_size = sub_range_x / TEMPLATE_X_SIZE;  // 416/60=6
  int y_block_size = sub_range_y / TEMPLATE_Y_SIZE;  // 150/20=7
  // ROS_INFO_STREAM("sub_range_x=" << sub_range_x);
  // ROS_INFO_STREAM("sub_range_y=" << sub_range_y);
  // ROS_INFO_STREAM("x_block_size=" << x_block_size);
  // ROS_INFO_STREAM("y_block_size=" << y_block_size);
  int pos;

  // initialize current_view
        ...
        y_block += y_block_size, y_block_count++)
    {
          ...
          x_block += x_block_size, x_block_count++)
      {
        ...
        for (int x = x_block; x < (x_block + x_block_size); x++)
        {
          ...
          {
            pos = (x + y * IMAGE_WIDTH);  // 2D -> 1D array pos
  ...
  else  // image->encoding == "bgr8" || image->encoding == "rgb8"
  {
    // ROS_INFO_STREAM("Not greyscale!");
    for (int y_block = IMAGE_VT_Y_RANGE_MIN, y_block_count = 0; 
        y_block_count < TEMPLATE_Y_SIZE; 
        y_block += y_block_size, y_block_count++)
    {
  ...
  {
    // ROS_INFO_STREAM("VT_NORMALISATION!");
  ...
  } // end if (VT_PATCH_NORMALISATION > 0)
