...
public:
  ...
  double x()
  {
    return best_x;  // updated by find_best()
  }
  double y()
  {
    return best_y;  // updated by find_best()
  }
  double th()
  {
    return best_th;  // updated by find_best()
  }

  // get and set all the cells as one array
  double * get_cells();  // return posecells_memory;
...
private:
  ...
  // locally excite and inhibit points. 
  // Excite spreads energy and
  // inhibit compresses.
  bool excite();
  bool inhibit();
  ...
  int PC_DIM_XY;
  int PC_DIM_TH;
  int PC_W_E_DIM;  // weight excite
  int PC_W_I_DIM;  // weight inhibit
  int PC_W_E_VAR;
  int PC_W_I_VAR;
