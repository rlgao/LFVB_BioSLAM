...
PosecellNetwork::PosecellNetwork(ptree settings)
{
   ...
   ** pose cell constants.
   */
  get_setting_from_ptree(PC_DIM_XY, settings, "pc_dim_xy", 21);  // 11
  ...
  get_setting_from_ptree(PC_VT_INJECT_ENERGY, settings, "pc_vt_inject_energy", 0.15);  // 0.1
  get_setting_from_ptree(PC_CELL_X_SIZE, settings, "pc_cell_x_size", 1.0);  // 0.015
...
void PosecellNetwork::pose_cell_builder()
{
  ...
  posecells_elements = PC_DIM_XY * PC_DIM_XY * PC_DIM_TH;  // number of pose cells
...
{
  if (act_x < PC_DIM_XY && act_x >= 0 && 
      act_y < PC_DIM_XY && act_y >= 0 && 
...
bool PosecellNetwork::global_inhibit()
{
  ...
  for (i = 0; i < posecells_elements; i++)
  {
    ...
    {
      posecells_memory[i] -= PC_GLOBAL_INHIB;
    }
    else
    {
...
int PosecellNetwork::generate_wrap(int * wrap, 
                                   int start1, int end1, 
                                   int start2, int end2, 
                                   int start3, int end3)
{
...
PosecellNetwork::PosecellAction PosecellNetwork::get_action()
{
  ...
  else  // experiences.size() != 0 && visual_templates.size() != 0
  {
...
void PosecellNetwork::on_view_template(unsigned int vt, double vt_rad)
{
  ...
  if (vt >= visual_templates.size())  // must be a new template
  {
    create_view_template();
    assert(vt == visual_templates.size()-1);
  }
  else  // the template must exist
  {
    pcvt = &visual_templates[vt];

    // inject energy from view template to pose cell
    if (vt < (visual_templates.size() - 10))  // this prevents energy injected in recently created vt's
    {
      if (vt != current_vt) { ; }
      else // vt == current_vt
      {
        pcvt->decay += VT_ACTIVE_DECAY;  // cause lower energy
      }
